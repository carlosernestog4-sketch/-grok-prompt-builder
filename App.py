...
st.set_page_config(page_title="Grok Prompt Builder v12.0 Nuclear 2025", layout="centered")
...
st.markdown("<h1>Grok Prompt Builder v12.0 Nuclear Ultimate 2025</h1>", unsafe_allow_html=True)
st.markdown("<p style='text-align: center; color: #aaa;'>Por Carlos Ernesto - Fondo Opcional en Detalles Extra</p>", unsafe_allow_html=True)
...
# Configuración Nuclear v12 (sin pestaña fondo)
st.markdown("### Configuracion Nuclear v12")

col1, col2 = st.columns(2)
with col1:
    tipo_plano = st.selectbox("Tipo de Plano v12 (frame infalible)", [
    ...
    lente_exacta = st.selectbox("Lente v12 (Canon / Sony / iPhone)", [
    ...
    iluminacion_pro = st.selectbox("Iluminacion Profesional v12", [
...
with col2:
    composicion_pro = st.selectbox("Composicion v12", ["rule of thirds golden ratio eye-level, subject 75% height (85% full body)", "centred symmetric", "low-angle dramatic", "high-angle natural"])
    usar_formula_nuclear = st.checkbox("Activar FORMULA NUCLEAR 2025 base (nunca falla)", value=False)

# Prioridad (máxima responsabilidad)
st.markdown("### Prioridad (máxima responsabilidad)")
prioridad_texto = st.text_area("Detalla aquí lo que quieres priorizar con máxima responsabilidad (ej: mujer de pelo suelto, mirada intensa)", value="", height=100)
prioridad_final = f"prioritise with maximum responsibility {prioridad_texto}, " if prioridad_texto.strip() else ""

# Detalles Extra Pro v12 (FONDO OPCIONAL AQUÍ)
expandir_detalles = st.checkbox("Activar Detalles Extra Pro v12 (incluye fondo opcional)", value=False)
detalles_base = st.text_area("Detalles Extra base v12 (escribe fondo aquí si quieres, ej: fondo de playa al atardecer)", value="mirada intensa directa, pose frontal relajada, camiseta negra ajustada, brillo natural en piel")
...
if expandir_detalles:
    ...
    st.success("Detalles Pro v12 ACTIVADOS")
else:
    detalles_final = detalles_base

# Fondo por defecto si no se escribe en detalles
...
# Salida principal
st.markdown("### Prompt Ingles Nuclear v12.0")
...
with col2:
    ...
    st.download_button("Descargar .txt Ingles", buffer_en, "prompt_ingles_v12.0.txt", "text/plain")

st.markdown("### Prompt Espanol Nuclear v12.0")
...
with col4:
    ...
    st.download_button("Descargar .txt Espanol", buffer_es, "prompt_espanol_v12.0.txt", "text/plain")

# Perfeccionador profesional
st.markdown("### Perfeccionador de Prompt Profesional v12.0")
...
st.markdown("<div class='footer'>Grok Prompt Builder v12.0 Nuclear Ultimate 2025 - Fondo Opcional en Detalles Extra - (c) Carlos Ernesto 2025</div>", unsafe_allow_html=True)
